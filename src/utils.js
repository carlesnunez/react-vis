...
export function sample(arr, len = 1) {
  let output = [];
...
export const roundTo = (value, places = 0) =>
...
export const camelToDashCase = (val) =>
  val.replace(/[A-Z0-9]/g, (letter) => `-${letter.toLowerCase()}`);
 ...
 */
export const splitCommaSeparatedArray = (value) => {
...
export const transformObject = (obj, callback) => {
  if (typeof obj !== "object") {
    return obj;
  }
...
export const replaceDigits = (value) => {
  return value
    .replace(/1/g, "one")
    .replace(/2/g, "two")
    .replace(/3/g, "three")
...
export const wrapCircularValue = (value, min, max) => {
...
export const mix = (v1, v2, ratio = 0.5) => v1 * (1 - ratio) + v2 * ratio;

export function mixAngles(angle1, angle2, ratio) {
